...
export default function Abouttwelve() {
  ...
  const startTimed = moment("8:00 AM", "h:mm A");
  const startTimes = moment("9:00 PM", "h:mm A");
  const timeSlotInterval = 30;
  const endTimed = moment("8:30 AM", "h:mm A");
  const endTimes = moment("9:30 PM", "h:mm A");
  ...
  const getTime = () => {
    ...
    while (currentTimeSlots <= startTimes) {
      const timeSlot = currentTimeSlots.format("h:mm A ");
  ...
  const getTime2 = () => {
    ...
    while (currentTimeSlot <= endTimes) {
      const timeSlots = currentTimeSlot.format("h:mm A ");
